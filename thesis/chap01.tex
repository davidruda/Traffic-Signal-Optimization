...
In this chapter, we introduce the optimization problem that we address in this thesis. Section~\ref{sec:competition_overview} briefly mentions the competition setting in which the problem was originally assigned. Section~\ref{sec:original_problem_statement} presents the original problem statement and describes it in detail. Section~\ref{sec:further_insights_and_heuristics} provides additional insights and heuristics that we use to simplify the problem and improve solutions. \xxx{Section~\ref{sec:simulator} ...}
...
\paragraph{Heuristic 2: Fix the Schedule for Trivial Intersections, Optimize Only Non-trivial Schedules} Trivial intersections do not require optimization; their schedule can be fixed by giving a green light to the only used incoming street for the entire duration. Only the schedules of non-trivial intersections need to be optimized. This can substantially reduce the number of optimized parameters. \\
...
\xxx{For more details on the code and implementation, please refer to the Developer Documentation in Appendix~\ref{chap:developer_documentation}.}
...
As explained in Section~\ref{sec:further_insights_and_heuristics}, these are the only schedules that require optimization.
