...
Simulation::Simulation(const CityPlan &city_plan) : city_plan_(city_plan) {
...
void Simulation::summary() const {
    ...
    const SimulationCar *earliest_car = nullptr;
    const SimulationCar *latest_car = nullptr;
    for (auto &&c: cars_) {
        if (c.finished()) {
            ++finished_count;
...
const std::vector<SimulationIntersection> &Simulation::intersections() const {
    return intersections_;
}

const std::vector<SimulationStreet> &Simulation::streets() const {
    return streets_;
}

const std::vector<SimulationCar> &Simulation::cars() const {
    return cars_;
}
