...
DATA = 'd'
PARALLEL = max(2, min(32, os.cpu_count()))

parser = argparse.ArgumentParser()
parser.add_argument('--data', default=DATA, type=str, help='Input data.')
parser.add_argument('--parallel', default=PARALLEL, type=int, help='Number of threads for parallel computation.')
...
class TestMultithreading(unittest.TestCase):
    ...
    def test_multithreading(self):
        _test_multithreading(self.data, self.parallel)


def _test_multithreading(data, parallel):
    def eval(_):
        return simulations[threading.get_ident()].score()

    def run_simulations(map_func):
        start = time.time()
        scores = list(map_func(eval, parallel * [0]))
        end = time.time() - start
        print(f'{parallel} simulations finished: {end:.4f}s')
        return scores, end
...
if __name__ == '__main__':
    ...
    times = _test_multithreading(args.data, args.parallel)
    if times:
